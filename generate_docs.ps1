...
# Categories mapping (Ordered for consistent output)
$Categories = [ordered]@{
...
function Get-ScriptDescription {
    ...
    if (Test-Path $FilePath) {
        $Content = Get-Content $FilePath -TotalCount 30
        
        $Purpose = $Content | Where-Object { $_ -match "^\s*(?:::|#|REM)?\s*Purpose:\s*(.*)" } | ForEach-Object { $matches[1].Trim() } | Select-Object -First 1
        $Requirements = $Content | Where-Object { $_ -match "^\s*(?:::|#|REM)?\s*Requirements:\s*(.*)" } | ForEach-Object { $matches[1].Trim() } | Select-Object -First 1
        $Usage = $Content | Where-Object { $_ -match "^\s*(?:::|#|REM)?\s*Usage:\s*(.*)" } | ForEach-Object { $matches[1].Trim() } | Select-Object -First 1

        $DescriptionParts = @()
        if ($Purpose) { $DescriptionParts += "**Purpose**: $Purpose" }
        if ($Requirements) { $DescriptionParts += "**Requirements**: $Requirements" }
        if ($Usage) { $DescriptionParts += "**Usage**: $Usage" }
...
# Function to get script type emoji
function Get-ScriptTypeEmoji {
    param ($FileName)
    ...
    switch ($Extension) {
        ...
        ".cmd" { return "📜 Batch" }
        ".sh" { return "🐧 Bash" }
        ".reg" { return "®️ Registry" }
        ".ahk" { return "⌨️ AutoHotKey" }
        ".txt" { return "📄 Text" }
        ".exe" { return "📦 Binary" }
...
foreach ($Category in $Categories.Keys) {
    ...
    $ReadmeContent += "| Type | Script | Description |`n"
    $ReadmeContent += "| :--- | :--- | :--- |`n"
    ...
    foreach ($Script in $Categories[$Category]) {
        ...
        if (Test-Path $ScriptPath) {
            ...
            $TypeEmoji = Get-ScriptTypeEmoji -FileName $Script
            $ReadmeContent += "| $TypeEmoji | [$Script]($Script) | $Description |`n"
