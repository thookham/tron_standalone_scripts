...
param (
	...
	[string]$HashDeep = "C:\Program Files\HashDeep\hashdeep.exe",                  # e.g. "$env:SystemRoot\system32\hashdeep.exe"
	...
	[string]$mktorrent = "C:\Program Files\mktorrent\mktorrent.exe",             # e.g. "R:\applications\mktorrent\mktorrent.exe"
	...
	[string]$MasterCopy = "C:\scripts\tron",          # e.g. "r:\utilities\security\cleanup-repair\tron"
	[string]$MasterCopyCygwinFormat = "/cygdrive/c/scripts/tron",  # e.g. "/cygdrive/r/utilities/security/cleanup-repair/tron/tron"
...
$OldVersion = Get-Content $SeedServer\$SeedFolderRS\tron\resources\functions\initialize_environment.bat -ea SilentlyContinue | Select-String -pattern "set TRON_VERSION"
$OldVersion = "$OldVersion".Split("=")[1]

# Extract release date of current version from the seed server and stash it in $OldDate
$OldDate = Get-Content $SeedServer\$SeedFolderRS\tron\resources\functions\initialize_environment.bat -ea SilentlyContinue | Select-String -pattern "set TRON_DATE"
...
# The "split" command/method is similar to variable cutting in batch (e.g. %myVar:~3,0%)
$NewVersion = Get-Content $MasterCopy\tron\resources\functions\initialize_environment.bat -ea SilentlyContinue | Select-String -pattern "set TRON_VERSION"
...
function main() {
	...
	Copy-Item $MasterCopy\* $SeedServer\$SeedFolderST\ -recurse -force
	log "   Done" darkgreen
	log "   Loading .torrent seed..." green
	mkdir "$SeedServer\$SeedFolderTorrent\Tron v$NewVersion ($CUR_DATE)" -ea silentlycontinue | out-null
	Copy-Item $MasterCopy\tron\* "$SeedServer\$SeedFolderTorrent\Tron v$NewVersion ($CUR_DATE)" -recurse -force
	log "   Done" darkgreen
	log "   Uploading .torrent to $TorrentAutoloaderLocation..." green
	Copy-Item "$TorrentSaveLocation\Tron v$NewVersion ($CUR_DATE).torrent" $SeedServer\$TorrentAutoloaderLocation
	if ($? -eq "True") {
		log "   Done" darkgreen
	}
	else {
	...
	start-job -name tron_copy_pack_to_seed_server -scriptblock { Copy-Item "$env:temp\$($args[0])" "$($args[1])\$($args[2])" -force } -ArgumentList $NewBinary, $SeedServer, $StaticPackStorageLocation
	""
